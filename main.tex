...
% Afficher / masquer les TODO
\newboolean{todo}
\setboolean{todo}{false}
...
\reviewer{Mme}{Anne}{Siegel}{Directrice de recherche CNRS}%
  {Inria Rennes (IRISA)}  %(CNRS \& Université Rennes 1)}

% Examinateurs
\president{Mme}{Mireille}{Régnier}{Directrice de recherche Inria}%
  {École polytechnique (LIX) \& Université Paris-Sud 11 (LRI)}

\examiner{M.}{Denis}{Thieffry}{Professeur des universités}%
  {École normale supérieure (Institut de biologie)}

% Encadrants
\supervisor{M.}{Olivier}{Roux}{Professeur des universités}%
  {École centrale de Nantes (IRCCyN)}

\examiner{M.}{Morgan}{Magnin}{Maître de conférences}%
  {École centrale de Nantes (IRCCyN)}
