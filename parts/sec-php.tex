...
dans l'utilisation des actions de priorité haute.



\subsection{Réutilisation des outils}

\todo{Revoir le titre de cette section}

\subsubsection{Points fixes}

Il est à noter que l'ensemble des points fixes des Frappes de Processus avec $k$
classes de priorités $\PH = (\PHs, \PHl, \PHh^{\angles{k}})$, où $k \in \sN^*$,
est identique à l'ensemble des points fixes des Frappes de Processus standards
$\PH' = (\PHs, \PHl, \PHh)$, c'est-à-dire du modèle dont l'ensemble des actions
est l'union de toutes les classes de priorités de $\PH$.
Ce résultat se déduit aisément du raisonnement qui a permis d'élaborer l'opérateur de jouabilité
de la \defref{fopphp}.
En effet, il existe une action jouable 
En effet, un point fixe est caractérisé par l'impossibilité de jouer toutes les actions ;
or, si une action est jouable dans $\PH'$, cela signifie que la même action dans $\PH$ :
\begin{itemize}
  \item soit qu'elle est jouable
\end{itemize}

\begin{theorem}[Point fixe des Frappes de Processus avec classes de priorités]
\thmlabel{fixedpoints-php}
  ...
  Une action est jouable dans un état $s$ de $\PH$ si et seulement si
  une action est jouable dans le même état $s$ de $\PH'$ :
  \[\exists s' \in \PHl, s \trans{\PH} s' \Leftrightarrow
    \exists s'' \in \PHl, s \trans{\PH'} s''\]
\end{theorem}
