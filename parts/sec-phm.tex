...
Nous montrons notamment que ces deux formalismes sont équivalents à l'aide de deux traductions
appropriées d'un formalisme vers l'autre.
...
De plus, les traductions permettent de naviguer entre les formalismes et d'en utiliser
...
Nous définissons ensuite le réseau d'automates synchronisés représentant des
Frappes de Processus avec actions plurielles (\defref{phm2an})
et nous montrons qu'il est bisimilaire (\thmref{bisimulationphm2an})

La \defref{phm2an} donne le réseau d'automates synchronisés $\phmtoan[\PH]$
correspondant à toutes
Frappes de Processus avec actions plurielles $\PH$.
La notation $\recouvre$ qualifie le recouvrement d'un ensemble de processus de sortes distinctes
par un autre comprenant des processus issus des mêmes sortes
(\defref{recouvrementps})
inspiré du recouvrement d'un état par un ensemble de processus
donné à la \defref[vref]{recouvrement}.
Le \thmref{bisimulationphm2an} montre que $\phmtoan[\PH]$ est bisimilaire à $\PH$.
...
\begin{definition}[Frappes de Processus d'un réseau d'automates ($\antophmsymbol$)]
\deflabel{an2phm}
