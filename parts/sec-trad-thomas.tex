...
$\PHdirectpredec{a}$ (\pref{eq:ph-direct-predec})
...
We define in \pref{eq:ph-predec} the set of predecessors of a sort $a$, that is, the sorts influencing $a$
...
Pour tout ensemble de composants $g \subset \components$ et tout sous-état
$\sigma \in \PHsubl_g$, on note $\ctx_g(\sigma)$ l'ensemble des processus frappant $a$
...
\\
\upsilon(\sigma) & \DEF
\upsilon(\sigma \uplus \etat{\upsilon'(\sigma) \mid 
...
\begin{split}\label{eq:edges-inference-noreg}
  \forall s \in \{ +, - \}, \quad\qquad\qquad \\
  a \xrightarrow{t+1} a \in \hat{E}_s \Longleftrightarrow\ & \PHpredecgene{a} = \emptyset \wedge \{ a_t, a_{t+1} \} \subset \PHl_a, \\
...
E_- &= \{ \arc{a}{-}{t}{b} \mid \nexists a \xrightarrow{t'} b \in \hat{E}_+
  \wedge t = \min \{l \mid a \xrightarrow{l} b \in \hat{E}_-\}\} \\
E_\pm &= \{ \arc{a}{\uns}{t}{b} \mid \exists a \xrightarrow{t'} b \in \hat{E}_+ \wedge \exists a \xrightarrow{t''} b \in \hat{E}_- \\
...
\begin{align*}
  E_+ &= \{\arcf{b}{+}{1}{a}, \arcf{c}{+}{1}{a}, \arcf{a}{+}{1}{a}, \arcf{b}{+}{1}{b}, \arcf{c}{+}{1}{c}\}\\
