...
Les liens d'équivalence entre les différents enrichissements des Frappes de Processus proposés
dans ce manuscrit sont résumés au sein de la \figref{contrib-equivalences},
où nous faisons aussi apparaître les liens avec d'autres formalismes discrets qui ont été
...
\begin{figure}[tp]
  ...
  \begin{tikzpicture}\scriptsize
    \path[use as bounding box] (-5.5,-4) rectangle (5.8,4.8);
    
    \draw[very thick, draw=black, fill=gray!10,yshift=-10] (-1.2,-2.5) ellipse (1.7 and 1)
      node[text width=4cm, align=center, yshift=-2]
      {Frappes de Processus\\standards};
    \draw[very thick, draw=gray, bend left=40,yshift=-20] (-4.5,-1.7) edge (2.5,-2.7);
    \path[draw, very thick, bend right=10,yshift=-10] (-1.1,-1.9) edge[->] (-.7,-.5);
    
    \draw[very thick, draw=blue, fill=blue!10] (0,2) ellipse (2.5 and 1.5)
      node[text width=4cm, align=center, yshift=.6cm]
      {Frappes de Processus\\avec classes de priorités};
    \draw[very thick, draw=red, fill=red!10] (0,1.3) ellipse (1.8 and .5)
      node[text width=4cm, align=center, yshift=-.1cm]
    ...
    \draw[very thick, draw=darkgreen, fill=darkgreen!10] (-3.5,0) ellipse (1.7 and 1)
      node[text width=4cm, align=center]
      {Frappes de Processus\\avec arcs neutralisants};
    \path[draw, very thick, bend right=10] (-2.1,1.6) edge[->] (-3,.5);
    \path[draw, very thick, bend right=10] (-2,0) edge[->] (-1,1.1);
    
    \draw[very thick, draw=violet, fill=violet!10] (3.5,0) ellipse (1.7 and 1)
      node[text width=4cm, align=center]
      {Frappes de Processus\\avec actions plurielles};
    \path[draw, very thick, bend left=10] (1.9,1.6) edge[<-] (2.7,.5);
    \path[draw, very thick, bend left=10] (2,0) edge[<-] (1,1.1);
    
    \node[equiv-externe]
      at (-4,3.5) {Modèle de Thomas\\Réseaux discrets};
    \path[draw, thick, bend left=5] (-2.6,3.5) edge[->] (-1.3,1.6);
    \path[draw, thick, bend right=5] (-2.7,3.3) edge[<-] (-1.5,1.5);
    
    \node[equiv-externe]
      at (4.5,2.5) {Automates\\synchronisés};
    \path[draw, thick, bend left=5] (4.6,2.1) edge[->] (4.3,0.6);
    \path[draw, thick, bend right=5] (4.3,2.1) edge[<-] (4,0.6);
    
    \node[equiv-externe]
      at (3,4.2) {Sémantique booléenne\\de Biocham};
    \path[draw, thick, bend right=5] (3,3.8) edge[->] (3.3,0.6);
    
    \node[equiv-externe]
      at (4,-2) {Réseaux de Petri bornés\\avec arcs inhibiteurs};
    \path[draw, thick, bend right=5] (4,-1.6) edge[<-] (3.7,-.7);
  \end{tikzpicture}
  }
  \caption{\figlabel{contrib-equivalences}%
    Relations entre les différents enrichissements des Frappes de Processus.
    Les nœuds rectangulaires représentent des formalismes discrets extérieurs aux
    Frappes de Processus tandis que
    les flèches représentent les différentes traductions formalisées dans cette thèse.}
\end{figure}



% Collaborations
\input{parts/collaborations}
