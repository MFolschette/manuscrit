...
\todo{Diluer cet exemple (cf. exemple à la fin de PHcanonique)}

\begin{example}
  Comparons les Frappes de Processus canoniques $\PH = (\PHs; \PHl; (\PHa^{(1)}; \PHa^{(2)}))$
  définies à la \figref{livelock} avec 
  ...
  sans délai des sortes coopératives, comme pour le modèle $\PH$ de la \figref{livelock}.
