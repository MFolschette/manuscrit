...
\todo{«~Successeurs~» ou «~fils~» ? Il est temps de choisir !}

\begin{demo}[\Thmref{approxinf}]
...
La \defref{lastprocs} définit $\lastprocs(\obj{a_i}{a_j})$ comme étant l'ensemble des ensembles
...
il existe un ensemble de processus $ps \in \lastprocs(P)$ tel que $ps \subset (s \play \delta)$.
...
\todo{Renommer $\lastprocs$ ? En $\f{derprocs}$ ?}

\begin{definition}[$\lastprocs : \Obj \to \powerset(\powerset(\Proc))$]
\deflabel{lastprocs}
  Pour tout objectif $\obj{a_i}{a_j} \in \Obj$, $\lastprocs(\obj{a_i}{a_j})$
  est défini comme le plus grand ensemble tel que :
  $\forall lps \in \lastprocs(\obj{a_i}{a_j}), lps \in \powerset(\Proc)$,
  \begin{enumerate}
    \item $a_j \in lps$,
    ...
    \item $\nexists lps'\in \lastprocs(\obj{a_i}{a_j}), lps' \varsubsetneq lps$.
  \end{enumerate}
\end{definition}
...
\thmlabel{approxinf-ordonnee}
  ...
  $\uconcr(P) \neq \emptyset \wedge \forall ps \in \lastprocs(P),
    \muconcr_{\ctx' \Cap ps}(\w') \neq \emptyset
    \Rightarrow \uconcr(\w) \neq \emptyset$,
  où $\ctx' = \ctx \Cap \procs(\mycwB{\ctx}{P})$.
\end{theorem}

\begin{demo}[\Thmref{approxinf-ordonnee}]
  Si $\uconcr(P) \neq \emptyset$, alors pour tout état $s \in \PHl, s \subset \ctx$,
  il existe un scénario $\delta \in \uconcr(P) \cap \Sce(s)$.
  D'après la \defref{lastprocs} et la démonstration du \stodo{ref : th:approxinf},
  il existe $ps \in \lastprocs(P)$ tel que $(s \play \delta) \subset \ctx' \Cap ps$.
