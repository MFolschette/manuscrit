...
\begin{example}
  ...
  Il s'agit du modèle de segmentation métazoaire précédemment proposé à la \vfigref{metazoan-ph}
  enrichi à l'aide de l'ensemble d'arcs neutralisants suivant :
  ...
  Ces arcs neutralisants permettent de s'assurer que l'évolution de la sorte $a$
  (son activation ou sa désactivation) primera toujours sur celle de la sorte $c$.
  Cela permet de se rapprocher du comportement observé biologiquement :
  $c$ possède un rôle d'horloge, et la modification de son processus actif doit
  être suivi de la modification du processus actif de $a$.
  L'ajout de ces deux arcs neutralisants permet d'introduire des contraintes locales dans
  le modèle.
  ...
  à d'autres endroits du modèles, comme cela pourrait être le cas avec des classes de priorités.
  
  \todo{exemple de dynamique}
...
\todo{Exemple de dynamique « parfaite »}
