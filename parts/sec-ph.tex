\section{Les Frappes de Processus}

\subsection{Définition des Frappes de Processus}
...
\deflabel{ph}
  Les \emph{Frappes de Processus avec $k$ classes de priorités} sont définies
...
\subsection{Sémantique des Frappes de Processus}
...
\begin{definition}[Propriété de jouabilité ($\F$)]
  \label{def:ppl}
  Une \emph{propriété de jouabilité} est un élément du langage $\F$ défini inductivement \todo{?} par :
  \begin{itemize}
    \item $\top$ et $\bot$ appartiennent à $\F$ ;
    \item si $a \in \ov{\PHs}$ et $a_i \in \ov{\PHl}_a$, alors $a_i \in \F$
      et $a_i$ est appelé un \emph{atome} ;
    \item si $P \in \F$ et $Q \in \F$,
      alors $\neg P \in \F$, $P \wedge Q \in \F$ et $P \vee Q \in \F$.
  \end{itemize}
%
  ...
  \begin{itemize}
    \item si $P = a_i \in \ov{\PHs}_a$ est un atome, avec $a \in \ov{\PHs}$,
  ...
  \end{itemize}
%
  Une fonction $\Fop : \PHh \rightarrow \F$ associant à toute action une propriété de jouabilité
...
\deflabel{fopph}
  ...
  \[\forall h \in \PHh, \Fop{h} \equiv \hitter{h} \enspace.\]
\end{definition}

\begin{definition}[Dynamique des Frappes de Processus ($\PHtrans$)]
\deflabel{play}
  Une action $h = \PHhit{a_i}{b_j}{b_k} \in \PHh$ est dite \emph{jouable}
  dans l'état $s \in \PHl$ si et seulement si : $\target{h} \in s \wedge \Feval{\Fop{h}}{s}$.
