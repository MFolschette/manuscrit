...
\tikzstyle{hl2}=[hl]
\tikzstyle{nohl}=[font=\normalfont,thin]
\tikzstyle{cooperativeactiondot}=[circle, fill=black, draw=black, inner sep=1]
...
\newcommand{\THit}[5]{
  \path[hit] (#1) edge[#2] node[pos=.5,cooperativeactiondot] {} (#3#4);
...
\newcommand{\TCoopHit}[6]{
  \node[#2, cooperativeactiondot] at (#3) {};
  \foreach \proc in {#1} {
    \draw[#2,-] (#3) edge (\proc);
  }
